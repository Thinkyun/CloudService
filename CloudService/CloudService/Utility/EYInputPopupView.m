...
@property (nonatomic, strong) PlaceholderTextView *tvContent;
            ...
            dismissBlock:(dispatch_block_t)dismissBlock
{
    ...
    popView.lbTitle.font = [UIFont boldSystemFontOfSize:16.0f];
    ...
    switch (type) {
        ...
        case EYInputPopupView_Type_multi_line:
        {
            ...
            popView.tvContent = [[PlaceholderTextView alloc] initWithFrame:CGRectMake((kAlertWidth - kContentWidth) * 0.5, CGRectGetMaxY(popView.lbTitle.frame)+kContentTopMargin, kContentWidth, kContentMinHeight)];
            ...
            popView.tvContent.placeholderColor = [HelperUtil colorWithHexString:@"#F4F4F4"];
            popView.tvContent.placeholder = @"请填写投保礼";
    ...
    CGRect leftBtnFrame = CGRectMake((kAlertWidth - 2 * kCoupleButtonWidth - kButtonBottomMargin-5) * 0.5, CGRectGetMaxY(popView.tvContent.frame)+kContentBottomMargin, kCoupleButtonWidth, kButtonHeight);
    CGRect rightBtnFrame = CGRectMake(CGRectGetMaxX(leftBtnFrame) + kButtonBottomMargin+5, CGRectGetMaxY(popView.tvContent.frame)+kContentBottomMargin, kCoupleButtonWidth, kButtonHeight);
...
- (void)rightBtnClicked:(id)sender
{
    ...
    if (self.confirmBlock) {
        self.confirmBlock(self,_tvContent.text);
    }
}
...
-(void)slipView:(BOOL)isUp offset:(float)offset{
    ...
    if (isUp) {
        [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
            self.transform = CGAffineTransformMakeRotation(0);
            self.frame  = CGRectMake((CGRectGetWidth(topVC.view.bounds) - kAlertWidth) * 0.5,
                                     topVC.view.bounds.origin.y+offset*4,
                                     self.frame.size.width,
                                     self.frame.size.height);
        } completion:^(BOOL finished) {
        }];
